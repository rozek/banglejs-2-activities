...
The theory behind this conversion may be found in [Wikipedia](https://en.wikipedia.org/wiki/Hue), although the Espruino function seems to use a different hue angle origin. What remains valid, however, is that you should find "pure" colors at hue angles which are multiples of 60° (i.e., 60°, 120°, 180°, ..., sometimes offset by 30°) if angles are measured in degrees (which is the common case). In the HSB model, "black" and "white" are not counted as "colors", but may be generated by setting "brightness" or "saturation" to 0, resp.
